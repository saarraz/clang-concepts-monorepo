...
// expected-error@-1 {{use of class template 'identity' requires template arguments; argument deduction not allowed here}}
static_assert(!requires { typename ::identity; });
// expected-error@-1 {{use of class template 'identity' requires template arguments; argument deduction not allowed here}}
static_assert(!requires { typename identity_t; });
// expected-error@-1 {{use of alias template 'identity_t' requires template arguments; argument deduction not allowed here}}
static_assert(!requires { typename ::identity_t; });
// expected-error@-1 {{use of alias template 'identity_t' requires template arguments; argument deduction not allowed here}}
...
namespace ns {
  using B = int;
  int C = 0;
  // expected-note@-1 {{struct 'C' is hidden by a non-type declaration of 'C' here}}
  ...
  // expected-error@-1 {{must use 'struct' tag to refer to type 'C' in this scope}}
}
...
static_assert(!requires { typename F<int>; }); // expected-error{{expected a type}}
static_assert(!requires { typename ::F<int>; }); // expected-error{{typename specifier refers to a non-type template}}
...
static_assert(requires { typename X::inner; });
// expected-error@-1{{reference to 'X' is ambiguous}}
